...
/// JoinChannelExternalAudio Example
class JoinChannelExternalAudio extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _State();
}

class _State extends State<JoinChannelExternalAudio> {
  ...
  _initEngine() async {
    ...
    await _engine.setExternalAudioSink(true, 16000, 1);
  }
  ...
  _addListeners() {
    _engine.setEventHandler(RtcEngineEventHandler(
      joinChannelSuccess: (channel, uid, elapsed) {
        ...
        // TODO Feed external audio via _engine.pushExternalAudioFrame
        setState(() {
          isJoined = true;
        });
      },
      ...
      userInfoUpdated: (uid, userInfo) {
        log('userInfoUpdated ${uid} User: ${userInfo.userAccount}');
      },
      userJoined: (uid, elapsed) {
        log('userJoined ${uid} elapsed: ${elapsed}');
      },
    ));
  }
  ...
  _joinChannel() async {
    ...
    await _engine
        .joinChannelWithUserAccount(config.token, config.channelId, config.stringUid)
        .catchError((onError) {
